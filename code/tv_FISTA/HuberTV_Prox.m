function [X_out,fun_all]=HuberTV_Prox(Bobs,gamma,P,center,lambda,l,u,pars)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Edited to have Huber penalty on A(x)-b
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% gamma..............................The huber function parameter
...
for i=1:MAXITER
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    D = reshape(A*Y(:),m,n)-Bobs;
    D = min(abs(D), gamma).*sign(D);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    Y=Y-2/L*reshape(A'*D(:),m,n);
    Y=real(Y);
    ...
    rx = A*Z_iter(:)-Bobs(:);
    fun_val=norm(rx-St(rx))+gamma*norm(St(rx),1) + lambda*t;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(mon==0)
        ...
        if(i>1)
            ...
            if(fun_val>fun_val_old)
                ...
                % Add in break condition
                if abs(fun_val - fun_val_old)/fun_val <= 0.001;
                    Break = 1;
                end
            end
        end
