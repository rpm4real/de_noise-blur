...
To illustrate this fact, we look at the orthogonal two-dimensional Haar wavelet transform of the peppers image. First we transform the image using one level. This results in an image divided into four parts: an image comprising row and column averages (scaled by $\sqrt{2}$), and three images made up of row, column, and diagonal differences. These three images are sparse, i.e. most of their pixels are zero or close to zero. Recursively applying this process, we get a Haar transform of the image using five levels, which is even more sparse. 
...
The Haar wavelet is just the first of many wavelets developed for multi-resolution analysis. In addition to the orthogonal two-dimensional Haar wavelet transform, we therefore also consider the orthogonal two-dimensional Daubechies wavelet transform in our examples. 
