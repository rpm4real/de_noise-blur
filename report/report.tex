...
\begin{document}
	...
	\begin{center}
		\begin{minipage}{0.8\textwidth}
			\begin{center}
				\textbf{Abstract}
			\end{center}
			We examine the optimization formulations which previous texts employ in denoising and deblurring images. In addition, we experiment with various different fidelity terms in an attempt to better capture more heavy-tailed noise. The algorithmic approach to these optimization formulations are outlined, and various parameters in the setup are tuned in search of optimum results. 
		\end{minipage}
	\end{center}
	...
	\begin{equation}
	\begin{aligned}
	x_{k+1} &= \prox_{\alpha^{-1}\lambda g}\big(x_k - \alpha^{-1} A^T \nabla f(Ax_k-b)\big) \\
	&= \min_y \frac{1}{2} \Big\| y - \big(x_k - \alpha^{-1} A^T \nabla f(Ax_k-b) \big)\Big\|_2^2 + \alpha^{-1}\lambda g(y)
	\end{aligned}
	\end{equation}
	...
	\begin{equation}
	\begin{aligned}
	x_{k} &= \prox_{\alpha^{-1}\lambda g}\big(y_k - \alpha^{-1} A^T \nabla f(Ay_k-b)\big) \\
	...
	applied to a real image using the discrete cosine and Haar wavelets. We corrupt each image with either high Gaussian or Student's t noise and blur the image. Additionally, we find an optimal lambda value for each image empirically and compare Huber and Frobenius norm fidelity functions. In figures \ref{waveletD_student} and \ref{waveletH_student} it can be seen that the Huber is able to better account for heavy-tailed noise, where as the Frobenius norm produces either blocky or smudged affects depending on the choice of wavelet basis.
	\begin{figure}[H]
		\begin{center}
